---
title: "Evolution of the R packages on CRAN using webscrapping"
author: [kevin]
date: 2020-05-20
...
http://dirk.eddelbuettel.com/blog/2017/01/07/
...
I revisited a script I wrote back in (actually 2015/11/20 in and last run R version 3.2.2 (2015-08-14)) with RCURL and even have a todo list :)
...
And I am gonna dealt with the three data frame differentlty.
...
Problem with this estimations:

if archived for sometime and the re-actualise some year without it correct!


So a better way to proceed is and `available()` and then to record changes. This is exacly what [](
http://dirk.eddelbuettel.com/cranberries/about/): every two hours now it records changes and keep that and all accessible. So... webscrap cranberries!
...
for (i in 2020) {
  ...
  cran_arcv0 <- readLines(con)
  close(con)
  ("http://dirk.eddelbuettel.com/cranberries/", i)
}
...
cran_pkgs_to_igraph <- function(mode = "all") {
  mata <- available.packages()

  pkg <- package_dependencies(available.packages()[, 1])
}
